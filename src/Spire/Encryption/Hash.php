...
class Hash
{
     ...
     * @return string
     */
    public static function make(string $data, string $algo = 'sha256'): string
    {
        // Check that we are using a valid algorithm.
        if (!self::validAlgo($algo))
        {
            throw new Exception(sprintf('Invalid hashing algorithm: %s.', $algo));
        }
